...
// Webshake Hotfix v1.5.2: Fallback-Changelog, falls Update-Daten fehlen
const webshakeChangelogFallback = [
  "Versionsanzeige / Changelog-Fix",
  "Paper-Trading sichtbar aktiviert",
  "BUY-/SELL-Regeln sichtbar gemacht",
  "Paper-Trades in Übersicht und Datenbank sichtbar",
  "Gewinn/Verlust seit Tradebeginn ergänzt"
];
...
const fallbackChangelog = [
  'v1.5.2 Beta: Versionsanzeige final ueber Backend/API synchronisiert.',
  'Changelog erscheint nach Updates wieder versionsgebunden.',
  'Paper-Trading ist sichtbar und aktiv pruefbar.',
  'BUY-/SELL-Regeln werden angezeigt.',
  'KI kann Paper-BUY/Paper-SELL simulieren, wenn Signal und Risiko passen.',
  'Wichtig: Es werden weiterhin keine echten Coinbase-Orders ausgefuehrt.'
];
...
function App(){
  ...
  const [showChangelog,setShowChangelog]=useState(()=>localStorage.getItem('webshake_changelog_v1_5_2_seen')!=='1');
  const [versionInfo,setVersionInfo]=useState(null);
  const [changelog,setChangelog]=useState({version:'v1.5.2 Beta', show_once_key:'webshake_changelog_v1_5_2_seen', notes:fallbackChangelog});
  ...
  async function loadFast(persist=false){
    ...
    if(st){
      setStatus(st);
      if(st.version_info) setVersionInfo(st.version_info);
      setError('');
    } else {
  ...
  async function loadSlow(){
    const [tr, tov, lo, de, ba, dbo, se, ps] = await Promise.all([
      ...
      safeGet('/paper/status', paperStatus)
    ]);
    if(tr) setTrades(tr);
  ...
  async function loadChangelog(){
    const cg = await safeGet('/changelog', null);
    if(cg){
      setChangelog(cg);
      const key = cg.show_once_key || `webshake_changelog_${String(cg.version||'').replace(/\W+/g,'_')}_seen`;
      if(localStorage.getItem(key)!=='1') setShowChangelog(true);
    }
  }
  async function load(){ await Promise.all([loadFast(true), loadSlow(), loadMonitor(), loadChangelog()]); }
  useEffect(()=>{
    load();
    const fast=setInterval(()=>loadFast(false),500);
    const persist=setInterval(()=>loadFast(true),10000);
    const slow=setInterval(loadSlow,5000);
    const mon=setInterval(loadMonitor,1200);
    return()=>{clearInterval(fast); clearInterval(persist); clearInterval(slow); clearInterval(mon)}
  },[]);
  ...
  function updateRestart(){ if(confirm('Update-Neustart starten?\n\nWebshake Trading beendet sich, sucht nach dem neuesten Update-Skript und startet nach dem Update neu.')) window.webshakeWindow?.updateRestart?.(); }

  return <div className="app">
    {showChangelog && <Changelog changelog={webshakeChangelogFallback} version={status?.version || versionInfo?.version || changelog?.version} onClose={()=>{localStorage.setItem(changelog?.show_once_key || 'webshake_changelog_v1_5_2_seen','1');setShowChangelog(false)}}/>}
    <div className="topbar"><div className="brand-centered"><div className="brand-title">Webshake Trading</div><div className="brand-version">{status?.version || 'Version wird geladen...'}</div></div><div className="window-actions"><button onClick={()=>window.webshakeWindow?.minimize?.()}>—</button><button onClick={()=>window.webshakeWindow?.maximize?.()}>□</button><button onClick={()=>window.webshakeWindow?.close?.()}>×</button></div></div>
    ...
    <main className="panel">
      ...
      {active==='Dashboard' && <Dashboard status={status} analysis={analysis} trades={trades} tradeOverview={tradeOverview} logs={logs} balance={balance} decisions={decisions}/>} 
      {active==='KI Analyse' && <AI analysis={analysis} decisions={decisions}/>} 
      {active==='KI Live-Monitor' && <LiveMonitor monitor={monitor} status={status}/>} 
      {active==='Live Trading' && <Trading trades={trades} tradeOverview={tradeOverview} reload={load} status={status} balance={balance} paperStatus={paperStatus}/>} 
      {active==='Risiko' && <Risk status={status} reload={load}/>} 
      {active==='Datenbank' && <DatabasePage db={db} trades={trades} logs={logs} decisions={decisions} settingsRows={settingsRows}/>} 
...
function Changelog({onClose, version}){return <div className="modal-backdrop"><div className="modal"><button className="modal-close" onClick={onClose}><X size={18}/></button><h2>Webshake Trading {version || 'v1.0.5 Beta'}</h2><p>Neue Version erfolgreich geladen.</p><ul>{webshakeChangelogFallback.map((c,i)=><li key={i}>{c}</li>)}</ul><button className="primary" onClick={onClose}>Verstanden</button></div></div>}
function Dashboard({status,analysis,trades,tradeOverview,logs,balance,decisions}){return <div className="grid">
  <Card icon={<Bot/>} title="KI Status"><p>{status?.risk?.emergency_stop?'SYSTEM GESTOPPT':'Bereit'}</p><small>{status?.risk?.emergency_stop?'Marktanalyse aktiv · Trading gesperrt':'Live-UI ca. 500 ms · KI-Speicherung ca. 10 s'} · Paper-Trading: {status?.paper_trading?'aktiv':'aus'}</small></Card>
  ...
  <Card icon={<TrendingUp/>} title="24H Hoch"><p className="positive">{euro(analysis?.tick?.high_24h_eur || analysis?.tick?.high_eur || analysis?.indicators?.high_24h_eur || analysis?.indicators?.high_eur)}</p><small>echter 24H-Wert · {analysis?.tick?.high_low_source || analysis?.indicators?.high_low_source || 'Coinbase'}</small></Card>
  <Card icon={<TrendingDown/>} title="24H Tief"><p className="negative">{euro(analysis?.tick?.low_24h_eur || analysis?.tick?.low_eur || analysis?.indicators?.low_24h_eur || analysis?.indicators?.low_eur)}</p><small>echter 24H-Wert · {analysis?.tick?.high_low_source || analysis?.indicators?.high_low_source || 'Coinbase'}</small></Card>
  ...
  <Card icon={<Wallet/>} title="Guthaben"><p>{euro(balance?.total_value_eur)}</p><small>EUR frei: {euro(balance?.available_eur)} · BTC: {balance?.btc_amount || 0}</small></Card>
  ...
  <div className="wide card"><h3>Aktuelle KI-Entscheidung</h3><p className="decision">{analysis?.action} · {(analysis?.confidence*100 || 0).toFixed(0)}%</p><p>{analysis?.reason}</p><div className="chips"><TrendBadge trend={analysis?.indicators?.trend}/><span>SMA kurz: {euro(analysis?.indicators?.sma_short)}</span><span>SMA lang: {euro(analysis?.indicators?.sma_long)}</span><VolatilityBadge value={analysis?.indicators?.volatility_20_samples_pct}/><span>gespeicherte KI-Daten: {decisions?.length || 0}</span></div></div>
...
function AI({analysis,decisions}){const [debug,setDebug]=useState(false); const indicators=analysis?.indicators || {}; return <div className="grid"><div className="wide card ai-summary"><div className="ai-head"><div><h3>KI Analyse</h3><p className="decision">{analysis?.action || 'WARTET'} · {((analysis?.confidence || 0)*100).toFixed(0)}%</p><p>{analysis?.reason || 'Die KI sammelt Live-Daten.'}</p></div><button className="mini" onClick={()=>setDebug(!debug)}>{debug?'Entwicklerdaten ausblenden':'Entwicklerdaten anzeigen'}</button></div><div className="grid mini-grid"><Card icon={<LineChart/>} title="Trend"><p><TrendBadge trend={indicators.trend}/></p><small>Kurz: {euro(indicators.sma_short)} · Lang: {euro(indicators.sma_long)}</small></Card><Card icon={<Activity/>} title="Volatilität"><p><VolatilityBadge value={indicators.volatility_20_samples_pct}/></p><small>20 Live-Datenpunkte</small></Card><Card icon={<ShieldAlert/>} title="Signalstärke"><p>{((analysis?.confidence || 0)*100).toFixed(0)}%</p><small>BUY / SELL / HOLD Bewertung</small></Card><Card icon={<Database/>} title="Strategie"><p>{analysis?.strategy || '-'}</p><small>Entscheidung wird gespeichert</small></Card></div>{debug && <div className="debugbox"><h3>Entwickler-Modus · Rohdaten</h3><pre>{JSON.stringify(analysis,null,2)}</pre></div>}</div><div className="wide card"><h3>KI-Entscheidungs-Historie</h3><DataTable rows={decisions} cols={['created_at','action','confidence','price_eur','trend','volatility_pct','reason']} /></div></div>}
function Trading({trades,tradeOverview,reload,status,balance,paperStatus}){const [amount,setAmount]=useState(1); async function trade(side){if(status?.risk?.emergency_stop){alert('Not-Aus ist aktiv. Marktanalyse läuft weiter, aber Trading ist gesperrt.'); return;} await fetch(`${API}/trading/paper/${side}?amount_eur=${Number(amount)||1}`,{method:'POST'}); reload()} return <div className="card"><h3>Paper Trading</h3><p>Live-Daten werden schnell aktualisiert. Die Oberfläche läuft automatisch live; KI-Speicherung bleibt getrennt. Echte Orders sind weiterhin gesperrt.</p><p><b>Status:</b> {status?.risk?.emergency_stop ? 'Not-Aus aktiv' : 'bereit'} · Paper-Gesamtwert: {euro(balance?.total_value_eur)}</p><p><b>KI Paper-Trading:</b> {paperStatus?.enabled ? 'aktiv' : 'wartet'} · Startkapital: {euro(paperStatus?.start_capital_eur || 50)} · Max offen: {paperStatus?.max_open_trades || 1}</p><div className="rules-grid"><div><b>BUY-Regeln</b><ul>{(paperStatus?.buy_rules||[]).map((r,i)=><li key={i}>{r}</li>)}</ul></div><div><b>SELL-Regeln</b><ul>{(paperStatus?.sell_rules||[]).map((r,i)=><li key={i}>{r}</li>)}</ul></div></div><label>Paper-Trade Betrag in €<input value={amount} onChange={e=>setAmount(e.target.value)} /></label><button className="primary" onClick={()=>trade('BUY')}>Paper-Buy testen</button><button className="secondary" onClick={()=>trade('SELL')}>Paper-Sell testen</button><h3>Gewinn/Verlust seit Tradebeginn</h3><DataTable rows={tradeOverview} cols={['created_at','side','entry_price_eur','current_price_eur','amount_eur','pnl_eur','pnl_pct','reason']} /><h3>Trade-Historie</h3><DataTable rows={trades} cols={['created_at','side','amount_eur','btc_amount','price_eur','reason']} /></div>}
...
function DatabasePage({db,trades,logs,decisions,settingsRows}){
  ...
  return <div className="card"><h3><Database/> Datenbank einsehen</h3>
    ...
    <DataTable rows={rows} cols={cols}/><p className="note">Löschen bleibt vorerst gesperrt, damit Analyse-Daten erhalten bleiben. Backups werden vor Updates automatisch erstellt.</p></div>}
