...
const changelog = [
  'v1.5.3 Reparatur: Frontend stabil wiederhergestellt.',
  'Styles und Hauptoberfläche aus stabiler Basis neu gesetzt.',
  'Changelog-Fehler im Frontend entfernt.',
  'Paper-Trading bleibt als Paper-System ohne echte Coinbase-Orders vorgesehen.',
  'Startsystem vereinheitlicht: start_webshake.bat ist jetzt der Hauptstarter.',
  'Update-System vereinheitlicht: update_webshake.bat und update_restart.bat nutzen dieselbe Logik.',
  'Alte Update-Verzeichnisse und veraltete Batchdateien werden automatisch bereinigt.',
  'Backup vor Cleanup bleibt aktiv; Datenbank, Logs und Einstellungen bleiben erhalten.',
  'KI Live-Monitor neu: zeigt live, was die KI gerade macht.',
  'Logging nutzt jetzt die Systemzeit des Windows-Servers.',
  'Not-Aus ist nach einmaligem Klick gesperrt; Marktanalyse läuft weiter.',
  'Volatilität im Dashboard farblich markiert: grün, gelb, rot.',
  'Changelog wird beim ersten Start der neuen Version angezeigt.',
  'Fenstergröße und Position werden gespeichert und beim nächsten Start wiederhergestellt.',
  'Datenbank: Einstellungen sichtbar, Log-Filter und deutsche Reiter.',
  'Update-Neustart-Button für schnelle Entwicklungsupdates.',
  'CMD-Fenster laufen standardmäßig versteckt; Anzeige später in den Einstellungen steuerbar.',
  'Start/Beenden der Software wird für Transparenz mitgeloggt.'
];
...
function App(){
  ...
  const [showChangelog,setShowChangelog]=useState(()=>localStorage.getItem('webshake_changelog_v1_5_3_seen')!=='1');
  ...
  async function loadSlow(){
    const [tr, tov, lo, de, ba, dbo, se] = await Promise.all([
  ...
  return <div className="app">
    {showChangelog && <Changelog onClose={()=>{localStorage.setItem('webshake_changelog_v1_5_3_seen','1');setShowChangelog(false)}}/>}
    <div className="topbar"><div className="brand-centered"><div className="brand-title">Webshake Trading</div><div className="brand-version">v1.5.3 Beta</div></div><div className="window-actions"><button onClick={()=>window.webshakeWindow?.minimize?.()}>—</button><button onClick={()=>window.webshakeWindow?.maximize?.()}>□</button><button onClick={()=>window.webshakeWindow?.close?.()}>×</button></div></div>
...
function Changelog({onClose}){return <div className="modal-backdrop"><div className="modal"><button className="modal-close" onClick={onClose}><X size={18}/></button><h2>Webshake Trading v1.5.3 Beta</h2><p>Neue Version erfolgreich geladen.</p><ul>{changelog.map((c,i)=><li key={i}>{c}</li>)}</ul><button className="primary" onClick={onClose}>Verstanden</button></div></div>}
function Dashboard({status,analysis,trades,tradeOverview,logs,balance,decisions}){return <div className="grid">
  <Card icon={<Bot/>} title="KI Status"><p>{status?.risk?.emergency_stop?'SYSTEM GESTOPPT':'Bereit'}</p><small>{status?.risk?.emergency_stop?'Marktanalyse aktiv · Trading gesperrt':'Live-UI ca. 500 ms · KI-Speicherung ca. 10 s'} · Paper-Trading: aktiv</small></Card>
